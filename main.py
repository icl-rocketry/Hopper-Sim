...
if __name__ == "__main__":
    ...
    for i in range(len(t)-1):
        ...
        temp = sc.integrate.solve_ivp(nlinsystem, (t[i], t[i+1]), x0, args=(u0,), method='RK45', events=(hit_ground))
        ...
        _, S, _ = np.linalg.svd(np.hstack([B(x0, u0), 
                                           A(x0, u0) @ B(x0, u0), 
                                           A(x0, u0) @ A(x0, u0) @ B(x0, u0), 
                                           A(x0, u0) @ A(x0, u0) @ A(x0, u0) @ B(x0, u0),
                                           A(x0, u0) @ A(x0, u0) @ A(x0, u0) @ A(x0, u0) @ B(x0, u0)]))
